...
### Deploy a holesky OFT peer

```bash
pnpm hardhat lz:deploy # follow the prompts
```

Note: If you are on testnet, consider using `MyOFTMock` to allow test token minting. If you do use `MyOFTMock`, make sure to update the `holeskyContract.contractName` in [layerzero.config.ts](./layerzero.config.ts) to `MyOFTMock`.
...
```bash
npx hardhat --network holesky-testnet send --dst-eid 40168 --amount <AMOUNT> --to <TO>
```
